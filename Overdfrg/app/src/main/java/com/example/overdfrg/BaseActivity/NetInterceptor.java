...
import static com.example.overdfrg.local_utils.NetworkUtils.isNetworkConnected;
...
//                .addInterceptor(new CommonParamsPostInterceptor())
    ...
    public OkHttpClient getClientWithCache() {
        return new OkHttpClient.Builder()
                .cache(new Cache(new File(Application1.mApplication.getCacheDir(), "NetCache10"), 10 * 1024 * 1024))
                .connectTimeout(20, TimeUnit.SECONDS)
                .readTimeout(20, TimeUnit.SECONDS)
                .writeTimeout(20, TimeUnit.SECONDS)
                .addInterceptor(getLogInterceptor())
                .addInterceptor(new CommonHeadersInterceptor())
                .addInterceptor(cacheInterceptor(Application1.getAppContext()))
                .addNetworkInterceptor(cacheInterceptor(Application1.getAppContext()))
                .sslSocketFactory(NetTrustManager.getNetTrustManager().createSSLSocketFactory()).hostnameVerifier(new NetTrustManager.TrustAllHostnameVerifier())
                .build();
    }
    ...
    static class CommonParamsInterceptor implements Interceptor {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            HttpUrl httpUrl = request.url()
                    ...
                    .addQueryParameter("token1", Application1.getApplication().mToken)
